...
- 📺 **双协议合一，一键直切**：
  ...
  - 🤖 **安卓电视 (Android TV)**：基于原生 **ADB 网络调试协议** 与无头输入法桥接，原生支持中文汉字、空格与 Emoji。
  - ⚡ **首页设备直切栏**：顶部并列显示所有可用电视，0 毫秒无缝秒切控制权，无需多层弹窗。
- 🎮 **全功能虚拟遥控器**：
  - 🧭 **十字方向盘 (D-Pad)**：上、下、左、右、中心 OK 确认键；
  - 🔘 **系统功能按键**：↩️ 返回 (Back)、🏠 主页 (Home)、☰ 菜单 (Menu)、🔇 静音；
  - 🔊 **媒体与音量**：音量 +、音量 -、播放 / 暂停；
  - 🧹 **文本辅助**：清空电视输入、退格删除、发送后自动回车。
- 🔐 **一次配对，永久免密**：
...
3. 点击右上角的 **⚙️ 设置图标**；
4. 在下方输入框填入 Apple TV 的内网 IP（例如 `192.168.1.50`），点击 **【发起配对】**；
